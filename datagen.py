...
class example_dataset(Dataset):
    ...
    def __init__(self, data_dir = cfg.example_data_dir, transform = None):
        self.files = os.listdir(os.path.join(data_dir, 'i_s'))
    ...
    def __getitem__(self, idx):
        ...
        i_t = io.imread(os.path.join(self.data_dir, 'i_t', img_name))
        i_s = io.imread(os.path.join(self.data_dir, 'i_s', img_name))
        h, w = i_t.shape[:2]
